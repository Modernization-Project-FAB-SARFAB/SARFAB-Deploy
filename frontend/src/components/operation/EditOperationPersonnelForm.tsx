...
import { EditOperationPersonnelFormProps } from "./types/EditOperationPersonnelFormProps";
...
export default function EditOperationPersonnelForm({
  operation,
  volunteers,
  military,
  register,
  errors,
  setValue,
}: EditOperationPersonnelFormProps) {
  ...
  useEffect(() => {
    if (operation && military && volunteers) {
      const initialResponsible = military.find(
        (m) => m.militaryId === operation.responsible.personId
      );
      setSelectedResponsible(initialResponsible?.militaryId || null);
      ...
      const initialPersonnel = operation.personnel
        .map((p) => {
          const militaryMember = military.find((m) => m.militaryId === p.personId);
          const volunteerMember = volunteers.find((v) => v.volunteerId === p.personId);
  
          if (militaryMember) {
            return {
              personId: militaryMember.militaryId,
              role: "Personal Militar",
              displayName: `${militaryMember.lastName} ${militaryMember.firstName}`,
              rank: militaryMember.rankName,
            };
          } else if (volunteerMember) {
            return {
              personId: volunteerMember.volunteerId,
              role: "Voluntario",
              displayName: `${volunteerMember.lastName} ${volunteerMember.firstName}`,
              rank: volunteerMember.gradeName,
            };
          }
  ...
  return (
    <div className="rounded-md border border-stroke bg-white p-6 shadow-md dark:border-strokedark dark:bg-boxdark">
      ...
      <div className="flex flex-col mb-4 mt-4">
        ...
        <div className="py-2 px-3 border border-stroke rounded-md dark:border-strokedark bg-gray-100 dark:bg-boxdark">
          {operation?.operationStatus === "Activa"
            ? "Activa"
